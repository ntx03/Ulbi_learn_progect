...
const ArticleRating = ({className, articleId}: ArticleRatingProps) => {
    ...
    const {data, isLoading, isError} = useGetArticleRating({
        articleId: articleId ?? '', userId: userData?.id ?? ''
    });
    const rating = data?.[0];

    const [rateArticleMutations] = useRateArticle();

    const handleRateArticle =  useCallback((starsCount: number | undefined, feedback?: string)=> {
        try{
    ...
    const onCancel = useCallback((starsCount: number | undefined)=> {
        handleRateArticle(starsCount)
    }, [handleRateArticle])

    const onAccept = useCallback((starsCount: number | undefined, feedback?: string)=> {
